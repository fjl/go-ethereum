...
def get_node_url_docker(path,port):
...
def start_network(nodes):
    for node in range(1,nodes+1):
        os.system('docker network create -d bridge node'+str(node)+'-network --subnet='+get_network(node)+'.0/24 --gateway='+get_network(node)+'.1')
