...
// This checks decoding of inputs with very deep nesting.
func TestStreamDepthLimit(t *testing.T) {
	var tests = []struct {
		depth   int
		limited bool
	}{
		{streamDepthLimit - 1, false},
		{streamDepthLimit, false},
		{streamDepthLimit + 1, true},
		{streamDepthLimit + 2, true},
	}

	for _, test := range tests {
		var x interface{}
		err := DecodeBytes(encodeNested(test.depth), &x)
		if test.limited {
			if err != ErrDepthLimit {
				t.Errorf("wrong error for depth %d: %v", test.depth, err)
			}
		} else if err != nil {
			t.Errorf("unexpected error at depth %d: %v", test.depth, err)
		}
	}
}

func encodeNested(depth int) []byte {
	var enc = []byte{0x80}
	var head [9]byte
	for i := 0; i < depth; i++ {
		lh := listhead{size: len(enc)}
		hb := reverse(lh.encode(head[:]))
		enc = append(enc, hb...)
	}
	return reverse(enc)
}
